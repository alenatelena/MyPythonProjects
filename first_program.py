...
def send_prompt_to_nn(textInput):
    ...
    pattern_user = f"Ты ассистент помогающий оптимизировать промпты пользователя, дополнять его деталями, задавая вопросы, на которые пользователь сможет ответить. Напиши какие вопросы можно задать к промпту: '{textInput}', чтобы дополнить его деталями и сделать более понятными. Верни только список вопросов \n"
